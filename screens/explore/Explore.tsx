...
const Explore: React.FC<ExploreProps> = ({ onSelectLanguage, onBack: propOnBack }) => {
  ...
  return (
    <div className="bg-[#000000] min-h-screen transition-colors duration-300">
      <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-8 pb-24">
        <header className="flex flex-col md:flex-row md:items-center gap-6">
          <button
            onClick={handleBack}
            className="self-start p-3 bg-slate-900 border border-slate-800 text-slate-400 hover:text-white hover:border-indigo-500/50 rounded-2xl transition-all flex items-center gap-2 group"
          >
            <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
            <span className="text-xs font-black uppercase tracking-widest pr-2">Back</span>
          </button>

          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">Browse Tracks</h1>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-900 border border-slate-800 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none text-white transition-all"
            />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLanguages.map((lang) => (
            <div
              ...
              className="group bg-slate-900 border border-slate-800 rounded-3xl p-6 hover:border-indigo-500/50 hover:bg-slate-800/50 transition-all cursor-pointer flex flex-col"
            >

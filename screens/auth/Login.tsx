...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMsg.includes('Failed to fetch') || errorMsg.includes('timeout') || errorMsg.includes('longer than usual')) {
        ...
        try {
          // Best-effort check if user exists in our profile table
          const userExists = await supabaseDB.findOne({ email });
          if (userExists) {
            errorMsg = 'Wrong password. Please try again.';
          } else {
            errorMsg = 'Account not found. Create an account to log in.';
          }
        } catch (e) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 overflow-y-auto transition-colors duration-300">
      <div className="min-h-full flex flex-col items-center justify-center px-6 py-8">
        <div className="w-full max-w-[340px] flex flex-col items-center">
          {/* Logo - Centered with NO spacing */}
          ...
          {/* Headline + Subtext - NO top padding/margin */}
          <div className="text-center pt-0 mt-0 pb-6">
          ...
          <div className="w-full space-y-5">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-500 transition-colors" />
                  <input
                    id="email"
                    type="email"
                    value={email}
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-white/5 border border-white/[0.08] rounded-xl text-white text-sm placeholder:text-slate-600 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:bg-slate-950 focus:outline-none transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-500 transition-colors" />
                  <input
                    id="password"
                    ...
                    disabled={isLoading || isGoogleLoading}
                    className="w-full pl-11 pr-11 py-3 bg-white/5 border border-white/[0.08] rounded-xl text-white text-sm placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 focus:bg-slate-950 focus:outline-none transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  />
                  <button
                    type="button"
            ...
            <button
              ...
              disabled={isLoading || isGoogleLoading}
              className="w-full py-3.5 bg-white/5 border border-white/[0.08] hover:border-slate-700 hover:bg-white/10 rounded-lg text-slate-200 text-sm font-medium flex items-center justify-center gap-3 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isGoogleLoading ? (
                <>
                  <Loader2 size={18} className="animate-spin" />

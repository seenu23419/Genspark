...
const Quiz: React.FC<QuizProps> = ({ questions: propQuestions, onComplete: propOnComplete }) => {
  ...
  if (!question && !isFinished) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col items-center justify-center p-4 text-center transition-colors duration-300">
        ...
        <button onClick={() => navigate(-1)} className="px-6 py-3 bg-blue-600 rounded-xl text-white font-bold transition-all flex items-center gap-2">
  ...
  if (isFinished) {
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col items-center justify-center p-6 overflow-hidden transition-colors duration-300">
        <div className="relative z-10 max-w-md w-full space-y-8 text-center animate-in fade-in duration-500">


          {isPassed && (
            <div className="flex justify-center pt-4">
              <div className="absolute w-32 h-32 bg-green-500 rounded-full blur-3xl opacity-20" />
              <div className="relative w-28 h-28 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center">
          ...
          <div className="space-y-3">
            <h1 className="text-4xl font-bold text-slate-900 dark:text-white">{isPassed ? 'Quiz Passed!' : 'Not Quite There'}</h1>
            <p className="text-slate-600 dark:text-slate-400 font-bold">
              {isPassed ? 'Lesson Unlocked' : `You need ${Math.ceil(questions.length * passPercentage / 100)} correct answers to pass`}
            </p>
          </div>

          <div className="bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-white/[0.08] rounded-2xl p-8 backdrop-blur-sm transition-colors duration-300">
            <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2">Your Score</p>
            <p className="text-5xl font-bold text-slate-900 dark:text-white">{Math.min(score, questions.length)} / {questions.length}</p>
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full py-5 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-2xl font-bold flex items-center justify-center gap-3 active:scale-95"
            >
  ...
  // 9. Main Quiz UI
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col pb-20 sm:pb-0 transition-colors duration-300">
      <div className="bg-gradient-to-b from-slate-50 dark:from-[#1e293b] to-white dark:to-slate-950 border-b border-slate-200 dark:border-white/[0.08] sticky top-0 z-40 transition-colors duration-300">
        <div className="max-w-full mx-auto px-6 py-8 w-full">
          <h1 className="text-xl sm:text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-6">Knowledge Check</h1>
          <div className="w-full h-1 bg-slate-100 dark:bg-white/[0.08] rounded-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto" ref={contentRef}>
        <div className="max-w-4xl mx-auto w-full px-6 py-8 flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-8">
            <p className="text-blue-500 font-bold text-xs uppercase tracking-widest">Question {currentQuestion + 1} of {questions.length}</p>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-mono font-bold text-sm border-2 ${timeLeft <= 10 ? 'border-red-500 text-red-500 dark:text-red-400' : 'border-blue-500 text-slate-700 dark:text-slate-300'}`}>
            ...
            <div className="quiz-question">
              <ReactMarkdown
                ...
                components={{
                  ...
                  code: ({ children, inline }: any) => inline ? <code className="bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-300 px-1.5 py-0.5 rounded text-sm font-mono">{children}</code> : <pre className="my-4 p-4 rounded-xl bg-slate-900 dark:bg-slate-950 border border-slate-200 dark:border-white/[0.08] text-emerald-500 dark:text-emerald-400 font-mono text-sm sm:text-base overflow-x-auto"><code>{children}</code></pre>
                }}
              >
                {question.text}
              </ReactMarkdown>
          ...
          <div className="space-y-3 mb-8">
            {question.options.map((option, index) => {
              ...
              let styles = "bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-white/[0.08] hover:border-blue-500 transition-colors duration-300";
              if (isSubmitted) {
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded flex items-center justify-center font-bold ${isSelected || (isSubmitted && isCorrect) ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-white/10 text-slate-600 dark:text-slate-400'}`}>
          ...
          {submittedAnswer !== null && question.explanation && (
            <div className={`mb-8 p-6 rounded-2xl border-l-4 ${isAnswerCorrect ? 'bg-green-500/5 border-green-500' : 'bg-blue-500/5 border-blue-500'}`}>
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 dark:text-white mb-2">Detailed Explanation</h4>
          ...
          <div className="pt-6 border-t border-white/10">
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 transition-all ${selectedOption === null ? 'bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-slate-600' : 'bg-blue-600 text-white scale-100 active:scale-95'}`}
            >

    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
      <div className="max-w-md w-full text-center space-y-8 animate-in zoom-in duration-500">
        <div className="relative">
          ...
          <div className={`relative w-32 h-32 rounded-full flex items-center justify-center mx-auto ${isPass ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-slate-800'
            }`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-[#1e293b] border border-white/[0.08] p-6 rounded-3xl">
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-1">Score</p>
            <h3 className={`text-2xl font-bold ${isPass ? 'text-white' : 'text-red-400'}`}>{score} / {total}</h3>
          </div>
          <div className="bg-[#1e293b] border border-white/[0.08] p-6 rounded-3xl">
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-1">Points Earned</p>
            <h3 className="text-2xl font-bold text-blue-400">+{isPass ? xp : 0} Points</h3>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          {isPass ? (
            <button
              onClick={onContinue}
              className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold text-lg flex items-center justify-center gap-3 hover:bg-emerald-500 transition-all active:scale-95"
            >
              <Terminal size={20} />
            ...
            <button
              onClick={onRetry}
              className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold text-lg flex items-center justify-center gap-3 hover:bg-blue-500 transition-all active:scale-95"
            >
              <RefreshCw size={18} />
              Retry Quiz
            </button>

    ...
    return (
        <div className="flex h-screen bg-slate-50 dark:bg-black text-slate-900 dark:text-slate-200 overflow-hidden font-sans selection:bg-indigo-500/30 transition-colors duration-300">
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                ...
                <main className={`flex-1 overflow-y-auto scroll-smooth relative no-scrollbar animate-in fade-in duration-300 bg-black transition-colors duration-300 ${(!hideBottomNav && !isKeyboardVisible) ? 'pb-[74px] md:pb-0' : ''}`}>
                    {children}
                </main>

                {/* Mobile Bottom Navigation - Improved visibility and safe-area support */}
                {!hideBottomNav && !isKeyboardVisible && (
                    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[999] bg-black/95 backdrop-blur-3xl border-t border-white/5 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_rgba(0,0,0,0.8)] py-1">
